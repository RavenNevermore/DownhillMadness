...
void FVictoryEdAlignMode::InputKeyReleased(FKey Key)
{
	...
	if(Key == EKeys::V)
	{
		...
		const FScopedTransaction Transaction( TEXT("Snap"), LOCTEXT( "VictoryGame", "Victory Vertex Snap" ), VictoryEngine->VSelectedActor->GetRootComponent()  );
		VictoryEngine->VSelectedActor->GetRootComponent()->Modify();
...
bool FVictoryEdAlignMode::InputKey(FLevelEditorViewportClient* ViewportClient,FViewport* Viewport,FKey Key,EInputEvent Event)
{
	//Middle Mouse
...
bool FVictoryEdAlignMode::InputAxis(FLevelEditorViewportClient* InViewportClient,FViewport* Viewport,int32 ControllerId,FKey Key,float Delta,float DeltaTime)
{
